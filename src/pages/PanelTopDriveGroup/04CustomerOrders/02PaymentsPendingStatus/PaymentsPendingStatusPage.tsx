...
import { getPaymentsPendingStatus } from '../../../../redux/LandingEcommerce/ordersSlice/actions';
// COMPONENTES
...
function PaymentsPendingStatusPage() {
  ...
  const { paymentsPending, loading } = useSelector((state: RootState) => ({
    ...
    <div>
      ...
      <div className={`${styles.container} d-flex align-items-start justify-content-center`}>
        ...
        <div className={`${styles.container__Component} p-4`}>
          ...
          {loading ? (
            <p>Cargando pagos pendientes...</p> // Mensaje de carga
          ) : (
            Array.isArray(paymentsPending) && paymentsPending.length > 0 ? (
              ...
              <p>No hay pagos pendientes.</p> // Solo mostrar si no hay resultados
            )
          )}
        </div>
      </div>
